...
export const setupEventListeners = (fetcher, profile, API_URL, auth) => {
    ...
    if (appointmentList) {
        appointmentList.addEventListener('click', async (e) => {
            // Usa .closest() para encontrar os botões de ação, não importa se o clique foi no ícone ou no texto.
            // Isso torna a área de clique mais robusta.
            const cancelButton = e.target.closest('.cancel-appointment-btn');
            const recordButton = e.target.closest('.open-record-btn');
            ...
            if (cancelButton) {
                ...
                const li = recordButton.closest('li');
                const medicalRecord = li.dataset.medicalRecord;

                // Preenche o modal com os dados da consulta
                recordAppointmentId.value = recordButton.dataset.id;
                recordPatientName.textContent = recordButton.dataset.patientName;
