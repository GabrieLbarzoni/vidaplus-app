...
document.addEventListener('DOMContentLoaded', () => {
    ...
    auth.onAuthStateChanged(async (user) => {
        ...
        if (user) {
            try {
                ...
                localStorage.setItem("firebaseToken", idToken); // Salva o token para uso em outras páginas
            ...
            localStorage.removeItem("firebaseToken"); // Limpa o token ao deslogar
    ...
    const initializeApp = (fetcher, profile) => {
        ...
        if (welcomeContainer && profile.name) {
            ...
            // Adiciona o link para o perfil apenas se o usuário for um profissional.
            // Isso garante que pacientes não sejam direcionados para a página de edição de profissional.
            if (profile.role === "professional") {
                welcomeHTML += `<a href="/profile.html" class="text-blue-600 hover:underline font-semibold">${profile.name}</a>`;
            } else {
                // Para pacientes, apenas exibe o nome, sem link.
                welcomeHTML += `<span class="font-semibold text-gray-800">${profile.name}</span>`;
            }
        ...
        if (logoutButton) {
            logoutButton.onclick = () => {
                // Limpa o token antes de deslogar para garantir que a sessão seja encerrada corretamente.
