    ...
    app.post("/professionals", authenticate, async (req, res) => {
        try {
            const {name, cpf, celular, especialidade, horarioInicio, horarioFim} = req.body;
            const uid = req.user.uid;
    ...
    app.put("/professionals/:id", authenticate, async (req, res) => {
        try {
            const {id} = req.params;
            const {name, celular, especialidade, horarioInicio, horarioFim} = req.body;
    ...
    app.post("/appointments", authenticate, async (req, res) => {
        try {
            ...
            // Valida se o horário está dentro do expediente do profissional e se não há conflitos
        ...
        try {
            ...
            const populatedAppointments = appointmentsData.map((app) => {
                ...
                return {
                    ...
                    medicalRecord: app.medicalRecord || null, // Garante que o campo exista
                };
            });
