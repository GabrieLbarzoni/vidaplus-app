    ...
    app.get("/api/user/status", authenticate, async (req, res) => {
        try {
            ...
            if (!patientDoc.empty) {
                return res.status(200).json({hasProfile: true, role: "patient"});
            }
            const professionalDoc = await db.collection("professionals").where("userId", "==", uid).limit(1).get();
            if (!professionalDoc.empty) {
                return res.status(200).json({hasProfile: true, role: "professional"});
            }
    ...
    app.post("/api/professionals", authenticate, async (req, res) => {
        try {
    ...
    app.get("/api/appointments", authenticate, async (req, res) => {
        try {
            const appointmentsSnapshot = await db.collection("appointments").orderBy("date", "asc").get(); const appointmentsData = appointmentsSnapshot.docs.map((doc) => ({id: doc.id, ...doc.data()}));
