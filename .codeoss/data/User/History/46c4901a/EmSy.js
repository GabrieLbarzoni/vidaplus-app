document.addEventListener("DOMContentLoaded", () => {
    ...
    // Garante que este script só execute na página de perfil (profile.html).
    // Se o container do perfil não existir, o script para aqui para evitar conflitos.
    if (!profileContainer) {
        return;
    }
    ...
    const toggleFieldsByRole = (role) => {
        ...
        // Alterna a visibilidade dos contêineres de campos
        patientFields.classList.toggle('hidden', !isPatient);
        professionalFields.classList.toggle('hidden', !isProfessional);

        // Alterna o estado 'disabled' e 'required' dos inputs para garantir
        // a validação correta do formulário. Inputs desativados não são validados.
        dataNascimentoInput.disabled = !isPatient;
        dataNascimentoInput.required = isPatient;

        especialidadeInput.disabled = !isProfessional;
        especialidadeInput.required = isProfessional;
        horarioInicioInput.disabled = !isProfessional;
        horarioInicioInput.required = isProfessional;
        horarioFimInput.disabled = !isProfessional;
        horarioFimInput.required = isProfessional;
    };

    // --- Lógica de Carregamento e Preenchimento da Página ---
    const initializeProfilePage = async (token) => {
    ...
    formProfile.addEventListener("submit", async (e) => {
        ...
        // Desativa o botão e mostra o status de carregamento para evitar cliques duplos.
        submitBtn.disabled = true;
        submitBtn.textContent = isEditMode ? "Salvando..." : "Cadastrando...";
 
        try {
            // Pega um token novo e válido no momento do envio.
            // Esta operação é assíncrona e pode falhar, por isso está dentro do try.
            ...
            if (role === 'patient') {
                ...
                data = {
                    name: nameInput.value,
                    celular: celularInput.value,
                    specialty: especialidadeInput.value, // Corrigido: o campo na API é 'specialty'
                ...
                throw new Error("Por favor, selecione um tipo de perfil.");
            }
            ...
            // Reativa o botão em caso de erro ou se o redirecionamento demorar.
            submitBtn.disabled = false;
    ...
    // Garante que o Firebase confirmou o estado de login antes de fazer qualquer coisa.
    firebase.auth().onAuthStateChanged(async (user) => {
        try {
            if (user) {
                // Usuário está logado, esconde o loader e mostra o conteúdo da página.
                if (loader) loader.classList.add('hidden');
                if (profileContainer) profileContainer.classList.remove('hidden');

                // Usuário está logado, busca o token e inicializa a página.
                const token = await user.getIdToken();
                initializeProfilePage(token);
            } else {
                // Usuário não está logado, redireciona para o login.
                // Atualiza a mensagem do loader antes de redirecionar.
                if (loader && loader.querySelector('p')) {
                    loader.querySelector('p').textContent = "Você não está autenticado. Redirecionando...";
                } else {
                    // Fallback caso o loader não exista ou a estrutura mude
                    showModal("Você não está autenticado. Redirecionando...", true);
                }
                setTimeout(() => (window.location.href = "/login.html"), 2000);
            }
        } catch (error) {
            console.error("Erro fatal na verificação de autenticação:", error);
            showModal("Ocorreu um erro ao verificar sua sessão. Tente recarregar a página.", true);
            
            // Garante que a UI não fique travada no estado de carregamento.
            if (loader) loader.classList.add('hidden');
            if (profileContainer) profileContainer.classList.remove('hidden');
            
            // Desabilita o formulário para evitar ações inesperadas.
            if (submitBtn) {
                submitBtn.disabled = true;
                submitBtn.textContent = "Erro de Autenticação";
            }
        }
    });
});