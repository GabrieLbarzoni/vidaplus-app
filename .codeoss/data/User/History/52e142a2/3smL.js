        ...
        if (user) {
            try {
                ...
                if (!userProfile.hasProfile) {
                    ...
                    // Adicionamos um try...catch para capturar erros durante a inicialização
                    // e evitar que a página inteira trave.
                    try {
                        initializeApp(userProfile);
                    } catch (initError) {
                        console.error("Erro CRÍTICO ao inicializar a aplicação:", initError);
                        showModal("Ocorreu um erro ao carregar a página. Funcionalidades podem estar indisponíveis. Verifique o console (F12).", true);
                    }
                }
            } catch (error) {
